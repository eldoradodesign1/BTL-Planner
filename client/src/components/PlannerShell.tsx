...
import { Bell, CalendarDays, CheckCircle2, ChevronDown, CircleHelp, Command, FolderKanban, Inbox, LayoutDashboard, Menu, MessageCircle, Plus, Search, Settings, Users, X, Sparkles, SlidersHorizontal, UserRound } from "lucide-react";
...
export function PlannerShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="planner-app min-h-screen overflow-hidden"><div className="lava-blob lava-blob-one" /><div className="lava-blob lava-blob-two" /><div className="lava-blob lava-blob-three" /><div className="ambient-orb ambient-orb-one" /><div className="ambient-orb ambient-orb-two" />
    <aside className={`planner-sidebar ${store.sidebarOpen ? "is-open" : "is-collapsed"}`} aria-label="Navigation principale">
      <div className="brand-lockup"><div className="brand-symbol"><img src={logoUrl} alt="Mon Essentiel" /></div>{store.sidebarOpen && <div><div className="brand-name">ME<span>.</span></div><div className="brand-caption">mon essentiel</div></div>}</div>
...
function ProfilePopover({ theme, setTheme, profileName, onNavigate }: { theme: ThemeName; setTheme: (theme: ThemeName) => void; profileName: string; onNavigate: (view: ViewName) => void }) { return <motion.div initial={{ opacity: 0, y: 8, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="popover-panel profile-popover"><div className="profile-popover-head"><div className="avatar avatar-self large">{profileName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</div><div><strong>{profileName}</strong><span>Mon Essentiel</span></div></div><div className="theme-label">Thème d’interface</div><div className="theme-grid">{themes.map((item) => <button key={item.id} className={`theme-chip ${theme === item.id ? "selected" : ""}`} onClick={() => setTheme(item.id)}><i style={{ background: item.swatch }} />{item.label}</button>)}</div><button className="popover-menu-item" onClick={() => onNavigate("profile")}><UserRound size={16} /> Mon profil</button><button className="popover-menu-item" onClick={() => onNavigate("settings")}><Settings size={16} /> Préférences</button></motion.div>; }
